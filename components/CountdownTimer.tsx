...
export default function CountdownTimer({ openedAt }: { openedAt: string }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setRemaining((prev) => {
        const next = secondsRemaining(new Date(openedAt));
        if (next <= 0 && prev > 0) setJustAlarmed(true);
        return next;
      });
    }, 1000);
  ...
  const alarmed = remaining <= 0;
  const urgent = !alarmed && remaining <= 15;
  ...
  return (
    <div className="flex justify-center">
      <div
        onAnimationEnd={() => setJustAlarmed(false)}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-mono text-xs font-medium ${
          alarmed
            ? "bg-coral-light text-coral-dark"
            : urgent
              ? "bg-coral-light text-coral-dark animate-pulse"
              : "bg-blue-light text-blue-dark"
        } ${justAlarmed ? "animate-alarm-shake" : ""}`}
      >
        {alarmed ? (
          <AlarmClock size={15} strokeWidth={2.5} className="shrink-0" />
        ) : (
        ...
        {alarmed ? "Time's up!" : `${minutes}:${seconds.toString().padStart(2, "0")} left to submit`}
      </div>
    </div>
  );
}
