...
export default function ReportButton({ captionId, className = "" }: Props) {
  ...
  async function handleToggleReport(e: React.MouseEvent) {
  ...
  return (
    <button
      ...
      aria-label={reported ? "Undo report" : "Report this caption"}
      title={reported ? "Click to undo" : "Report this caption"}
    >
